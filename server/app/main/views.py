...
from .data import Restaurant, getElement, getPic, getRecommendation

@main.route('/recommendation', methods=['GET', 'POST'])
def recommendation():
    if request.method == 'GET':
        return jsonify(getPic())
    else:
        return jsonify(getRecommendation(request.data))
...
def index():
    ...
    if show_followed == 1:
        ...
        users = None
        restaurants = [Restaurant(getElement(bid)) for bid in current_user.recommendation.split(',')]
        pagination = None
        # print restaurants
    else:
        query = User.query
    ...
    return render_template('index.html', users=users, show_followed=show_followed, pagination=pagination, restaurants=restaurants)
...
@main.route('/recommendation_for_user')
@login_required
