...
class User(UserMixin, db.Model):
    ...
    def loadFromJson(k):
        ...
        import datetime
        i = 0
        with open('/Users/LZY3QY/CC data/processed_data/users_lv.json') as users:
            for user in users:
                i += 1
                if i == k: break
                user = json.loads(user)
                u = User(email=user['user_id'] + '@CC.com',
                         ...
                         member_since=datetime.datetime.strptime(user['yelping_since'], '%Y-%m-%d'))
                db.session.add(u)
        try:
            db.session.commit()
        except:
...
class Post(db.Model):
    ...
    business_id = db.Column(db.String(64))
    ...
    def loadFromJson():
        ...
        import json
        import datetime
        with open('/Users/LZY3QY/CC data/processed_data/reviews_lv.json') as reviews:
            for review in reviews:
                review = json.loads(review)
                u = User.query.filter_by(username=review['user_id']).first()
                if not u: continue
                p = Post(body=review['text'],
                         timestamp=datetime.datetime.strptime(review['date'], '%Y-%m-%d'),
                         author=u,
